...
- **Rolling AODB list window (today only)** — For today's view, AODB-backed flight lists (dropdowns/tables/API feeds) start from current system time minus 2 hours; past/future dates retain full-day history.
...
- AODB list filtering uses a rolling window of now minus 2 hours when viewing today; other dates show full-day lists
